...
class FactotumClient(py9p.Client):
    ...
    @contextmanager
    def _connection(self, *args: Any, **kwargs: Any) -> Generator[object, None, None]:
        try:
        ...
        with self._connection(_File.RPC.value, py9p.ORDWR):
            try:
                if "proto" in kwargs.keys() and kwargs["proto"] != "pass":
                    raise FactotumClientError("getpass", "wrong proto", kwargs["proto"])
                else:
                    kwargs["proto"] = "pass"
                if "role" in kwargs.keys() and kwargs["role"] != "client":
                    raise FactotumClientError("getpass", "wrong role", kwargs["role"])
                else:
                    kwargs["role"] = "client"
